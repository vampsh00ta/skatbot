...
func (h BotHandler) LikeAction() tgbotapi.HandlerFunc {
	return func(ctx context.Context, b *tgbotapi.Bot, update *tgmodels.Update) {
		...
		if err != nil {
			h.log.Error().Str("LikeAction", "error").Msg(err.Error())
		...
		if variantUserId != int(userId) {
			...
			h.log.Error().Str("LikeAction", "error").Msg("no such variant to delete")
		...
		h.log.Info().Str("LikeAction", "ok").Str("variantId", strconv.Itoa(variantId)).
